...
function renderMarkdownError(messageKey: string) {
  renderedHtml.value = `<p class="text-red-500">${escapeHtml(t(messageKey))}</p>`
}
...
async function fetchAndRenderMarkdown(slug: string) {
  ...
  try {
    ...
    if (!resp.ok) {
      renderMarkdownError('customPage.markdownNotFound')
      return
    }
    ...
    renderMarkdownError('customPage.markdownLoadFailed')
  } finally {
    loading.value = false
    await nextTick()
    await nextTick()
...
function injectCopyButtons() {
  const container = markdownContainer.value
  if (!container) return
  const defaultLabel = t('customPage.copyCode')
