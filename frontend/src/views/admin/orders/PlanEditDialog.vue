    ...
    <form id="plan-form" @submit.prevent="handleSavePlan" class="space-y-4">
      ...
      <div>
        ...
        <textarea
          ...
          data-testid="plan-product-name-input"
          rows="2"
          class="input"
          :placeholder="t('payment.admin.productNamePlaceholder')"
        ></textarea>
        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ t('payment.admin.productNameHint') }}</p>
      </div>
      <div class="grid grid-cols-2 gap-4">
...
const planForm = reactive({ name: '', group_id: null as number | null, description: '', product_name: '', price: 0, original_price: 0, validity_days: 30, validity_unit: 'days', sort_order: 0, for_sale: true })
...
watch(() => props.show, (visible) => {
  if (!visible) return
  if (props.plan) {
    Object.assign(planForm, { name: props.plan.name, group_id: props.plan.group_id, description: props.plan.description, product_name: props.plan.product_name || '', price: props.plan.price, original_price: props.plan.original_price || 0, validity_days: props.plan.validity_days, validity_unit: props.plan.validity_unit || 'days', sort_order: props.plan.sort_order || 0, for_sale: props.plan.for_sale })
    planFeaturesText.value = (props.plan.features || []).join('\n')
  } else {
    Object.assign(planForm, { name: '', group_id: null, description: '', product_name: '', price: 0, original_price: 0, validity_days: 30, validity_unit: 'days', sort_order: 0, for_sale: true })
